...
**매수 실행:**
- 조건 충족 시 → 돌파 기준선 가격에 시장가 매수

**매도 조건:**
- 매수 후 다음 6시간 봉 시가에 시장가 매도
...
### 3.1 성능
- 6시간 봉 마감 후 1분 내에 매매 조건 검증 및 실행
...
### 매수 조건 1: 돌파 기준선 돌파
```
현재봉 고가 > 현재봉 시가 + (전봉 고가 - 전봉 저가) × 0.5
```
- 이 조건은 현재 봉이 전 봉의 변동폭의 50% 이상 상승했는지 확인
- 돌파 기준선 = 현재봉 시가 + (전봉 고가 - 전봉 저가) × 0.5

### 매수 조건 2: 5봉 종가 평균 상회
```
돌파 기준선 가격 > 최근 5봉 종가 평균
```
- 이 조건은 현재 가격이 최근 5봉의 평균 가격보다 높은지 확인
- 추세 확인을 위한 필터
...
전봉 거래량 < 현재봉 거래량
```
- 이 조건은 현재 봉의 거래량이 전 봉보다 증가했는지 확인
- 돌파의 신뢰성 확보를 위한 필터
