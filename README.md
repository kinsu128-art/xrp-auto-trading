...
| 지표 | 결과 |
|---|---|
| 총 수익률 | 백테스트 실행 후 업데이트 |
| 연간 수익률 | 백테스트 실행 후 업데이트 |
| 승률 | 백테스트 실행 후 업데이트 |
| 손익비 | 백테스트 실행 후 업데이트 |
| 최대 손실률(MDD) | 백테스트 실행 후 업데이트 |
| 샤프 비율 | 백테스트 실행 후 업데이트 |
| 총 거래 횟수 | 백테스트 실행 후 업데이트 |

> 초기 자본 100만 KRW, 수수료 0.25% 적용 기준
> 인트라데이 방식: 다음 봉 고가 ≥ 돌파기준선 → 돌파기준선 가격에 매수 시뮬레이션
...
캔들 마감 후 다음 봉 형성 중 돌파기준선 도달 시 즉시 매수합니다.
...
**① 캔들 마감 시 사전 조건 확인 (조건 2 & 3)**

| 조건 | 내용 |
|---|---|
| 조건 2 (추세 확인) | 돌파기준선 > 최근 5봉 종가 평균 |
| 조건 3 (거래량 확인) | 전봉 거래량 < 현재봉 거래량 |

조건 2 & 3이 모두 충족되면 다음 봉에 대한 인트라데이 감시를 시작합니다.

**② 인트라데이 감시 (매 60초, 다음 봉 형성 중)**

```
현재가 ≥ 돌파기준선 → 즉시 지정가 매수 주문 접수
```

봉 마감까지 도달하지 못하면 감시 만료 → 다음 봉에서 재판단합니다.

#### 매도 조건

마감캔들 종가가 최근 5봉 종가 평균 미만이면 매도합니다. (거래량 조건 제외)

```
마감캔들 종가 < 최근 5봉 종가 평균 → 시장가 매도
```

> 텔레그램 알림:
> - 감시 시작 시: `👁 인트라데이 감시 시작 (돌파기준선: X원)`
> - 매수 체결 시: `✅ 매수 체결 완료 (지정가: X원)`
> - 감시 만료 시: `⏰ 돌파 없이 봉 마감`
> - 매도 시: `📋 매도 사유: 종가(X) < 5봉평균(Y)`

#### 스케줄

한국시간 기준 **00:00, 06:00, 12:00, 18:00** 에 캔들 마감 체크 및 매도/감시 설정 실행
...
| 버전 | 날짜 | 내용 |
|---|---|---|
| 1.6.0 | 2026-03-03 | 매도 조건 변경 - 돌파기준선 기준 → 마감캔들 종가 < 5봉 종가 평균 기준 |
| 1.5.0 | 2026-02-27 | 인트라데이 돌파 전략으로 전환 - 캔들 마감 후 다음 봉 형성 중 돌파기준선 도달 시 지정가 매수. 산식: 현재봉 종가 + (고가-저가)×0.5. 매도 조건 단순화(조건2만 체크) |
| 1.4.0 | 2026-02-26 | 돌파 기준선 판단을 고가→종가로 변경 - 백테스트 수익률 +816.53% 달성 (MDD 1.21%) |
| 1.3.0 | 2026-02-26 | 매도 조건에서 거래량 확인 제외 - 백테스트 수익률 +278.46% 달성 (MDD 10.29%) |
